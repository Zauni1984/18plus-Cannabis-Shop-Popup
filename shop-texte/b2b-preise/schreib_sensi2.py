...
plan=json.load(open(S+'sensi_titel_plan.json'))
...
for x in plan:
    if x['art']=='variation': nach_eltern[x['parent']].append(x)
...
for i,(pid,zeilen) in enumerate(nach_eltern.items()):
    try:
        ...
        for x in zeilen:
            ...
            if e and not e.get('error') and geprueft(e,x):
                ok+=1; beleg[x['sku']]=[x['id'],x['b2b'],x['anbauverein']]
            else:
    ...
    if i%25==0: print(f'  {ok} ok, {fehler} Fehler', flush=True)
    time.sleep(0.3)
json.dump(beleg, open(S+'sensi_titel_beleg.json','w'), ensure_ascii=False)
