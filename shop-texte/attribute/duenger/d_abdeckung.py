...
for slug in sorted([x for x in A.ATTR_ID if c[x]], key=lambda s: -c[s]):
    print(f'{slug:24} {c[slug]:8} {c[slug]*100//n:9} %')
json.dump({'anzahl': n, 'abdeckung': dict(c)}, open('d_abdeckung.json', 'w'), ensure_ascii=False, indent=1)
