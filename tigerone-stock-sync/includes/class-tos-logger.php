...
class TOS_Logger {
	...
	public static function purge() {
		global $wpdb;
		$days = max( 1, (int) TOS_Settings::get( 'log_keep_days', 30 ) );
		$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . TOS_Install::log_table() . ' WHERE ts < DATE_SUB(NOW(), INTERVAL %d DAY)', $days ) );
	}

	public static function recent( $limit = 300 ) {
		global $wpdb;
		return $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . TOS_Install::log_table() . ' ORDER BY id DESC LIMIT %d', (int) $limit ) );
	}
}
